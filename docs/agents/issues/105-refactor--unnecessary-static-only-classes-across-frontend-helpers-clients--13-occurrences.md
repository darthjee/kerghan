...
## Description
Codacy's ESLint `@typescript-eslint/no-extraneous-class` rule flags 15 classes across the codebase whose bodies contain only static members (or, for one, an empty body): 14 in `frontend/` (HTTP/event-bus "client" wrappers and JSX rendering "helper" classes) and 1 in `backend/` (`backend/src/core/logging.module.ts:28`, an empty `LoggingModule` body decorated with `@Module(...)`). (The original finding's title/summary undercounts this as "13" — the actual list of flagged frontend files below has 14 entries.)
...
## Solution
Bring the remaining 11 flagged frontend files in line with the existing, deliberate convention rather than converting them to plain modules:
...
## Benefits
- Clears all 14 Codacy findings for this pattern in `frontend/` without changing any public import paths or call-site ergonomics.
- Makes the codebase's deliberate static-class convention for client/event-bus/helper wrappers explicit and consistent everywhere it's used, instead of only in 3 of 14 files.
